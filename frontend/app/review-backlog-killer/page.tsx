...
const KEY = 'backlog' as const;
const URL = 'https://agena.dev/review-backlog-killer';

const BACKLOG_KEYWORDS = [
  'PR review backlog automation', 'auto-nudge PR reviewer',
  'stuck pull request alert', 'PR review SLA Slack',
  'GitHub PR reviewer reminder', 'Azure DevOps PR review nudge',
  'engineering velocity tool', 'review queue dashboard',
];
...
export default function ReviewBacklogKillerPage({ searchParams }: { searchParams: { lang?: string } }) {
  const cookieLang = cookies().get('agena_lang')?.value;
  const lang = pickLang(searchParams?.lang, cookieLang);
  const c = landingCopy(KEY, lang);
  const ldJson = {
    '@context': 'https://schema.org', '@type': 'SoftwareApplication',
    name: 'AGENA — Review Backlog Killer', applicationCategory: 'DeveloperApplication',
  ...
  return (
    <main style={{ maxWidth: 980, margin: '0 auto', padding: '40px 20px', display: 'grid', gap: 48 }}>
      ...
      <header style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#f59e0b', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 12 }}>{c.eyebrow}</div>
        <h1 style={{ fontSize: 'clamp(28px, 5vw, 46px)', fontWeight: 800, lineHeight: 1.1, color: 'var(--ink-90)', margin: 0 }}>
          {c.h1A} <br />
          <span style={{ background: 'linear-gradient(90deg, #f59e0b, #ef4444)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>{c.h1B}</span>
        </h1>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 24, flexWrap: 'wrap' }}>
          <Link href='/signup' style={{ padding: '12px 24px', borderRadius: 10, background: 'linear-gradient(135deg, #f59e0b, #ef4444)', color: '#fff', fontSize: 14, fontWeight: 700, textDecoration: 'none' }}>{c.ctaPrimary}</Link>
          <Link href='/dashboard/review-backlog' style={{ padding: '12px 24px', borderRadius: 10, border: '1px solid var(--panel-border)', background: 'var(--panel)', color: 'var(--ink)', fontSize: 14, fontWeight: 700, textDecoration: 'none' }}>{c.ctaSecondary}</Link>
        </div>
      </header>
      <section>
      ...
      <footer style={{ textAlign: 'center', padding: '40px 0', borderTop: '1px solid var(--panel-border)' }}>
        ...
        <Link href='/signup' style={{ padding: '12px 28px', borderRadius: 10, background: 'linear-gradient(135deg, #f59e0b, #ef4444)', color: '#fff', fontSize: 14, fontWeight: 700, textDecoration: 'none', display: 'inline-block' }}>{c.footerCta}</Link>
      </footer>
    </main>
  );
}
