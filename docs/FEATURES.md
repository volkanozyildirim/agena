...
### Runtime Context Resolution (What a task checks before code generation)

When a task starts, orchestration builds a single effective context by combining these sources:

1. Task core details
- `title`, `description`
- story fields: `story_context`, `acceptance_criteria`, `edge_cases`

2. Mapping / repo metadata
- `Local Repo Path` and mapping info from task description metadata
- repository top-level context (detected stack markers, directories, key files)

3. Repo Profile (from mapping scan)
- profile generated by `/preferences/repo-profile/scan`
- includes stack, suggested test/lint commands, repo rules, scan metadata

4. Repo AGENTS.md guidance
- latest generated/edited AGENTS doc for that mapping
- stored in app-managed path under `data/repo_agents/...`
- injected into prompt context as repository execution guidance

5. Repo Playbook (mapping-level)
- mapping-specific `repo_playbook` rules
- intended for repo-only conventions and constraints

6. Tenant Playbook (organization-level / global for that tenant)
- integration provider `playbook` content
- organization-wide engineering/governance rules

All of the above are merged into the final prompt context before `analyze -> generate_code -> review_code`.
