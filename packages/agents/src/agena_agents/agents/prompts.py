...
REFINEMENT_SYSTEM_PROMPT = (
    ...
    '  Anchor by scope, NOT by description length — a sparse description for a vague new\n'
    '  initiative (e.g. "Yeni X özelliği", "X integrasyonu") is a STRONG signal of bigness,\n'
    '  not smallness:\n'
    '    • trivial UI tweak / small bug                : 1-2\n'
    '    • single-screen feature, well-bounded         : 3-5\n'
    '    • cross-screen feature, multi-API touch       : 8-13\n'
    '    • new product capability, multi-team / vague  : 13-21 + breakdown recommended\n'
    '  When you suggest 13 or more, call out IN THE COMMENT that the item should be split\n'
    '  into smaller deliverables before sprint commitment.\n'
    '- Confidence reflects how well you actually understand the work, NOT how confident\n'
    '  you want to sound. If acceptance criteria are missing or the description is one\n'
    '  paragraph for a multi-system feature, confidence should be LOW (20-45%) — never\n'
    '  inflate it to look decisive. Confidence ≥ 70% requires concrete acceptance\n'
    '  criteria + clear scope.\n'
    '- For ambiguous / new / large items, generate AT LEAST 5 specific follow-up\n'
    '  questions covering: scope boundary, acceptance criteria, integrations, data\n'
    '  requirements, edge cases, rollout, monitoring. Do not stop at 1-2 surface\n'
    '  questions. Each question must be answerable by the PO/PM.\n'
    '- The "comment" field must be a direct, actionable refinement note — NOT a question.\n'
    '  It should be ready to paste as a work item comment. For SP >= 13, also include a\n'
    '  short bullet list of suggested sub-tasks the team could split this into.\n'
...
REFINEMENT_DESCRIPTION_PROMPT = (
    ...
    'If the description is sparse, do NOT default to a low SP — sparse description on a\n'
    'broad-sounding title (new initiative, new integration, new capability) means the\n'
    'item is under-specified, which is itself a signal that scope is large and unknown.\n'
    'In that case: estimate generously (8-21), keep confidence low (20-45%), and produce\n'
    'a rich list of clarifying questions and ambiguities (5+ each) so the team can split\n'
    'and re-refine before commitment.\n'
