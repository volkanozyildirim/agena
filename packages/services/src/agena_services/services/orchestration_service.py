...
def _format_pr_title(
    *,
    ...
    az_m = _AZURE_TITLE_PREFIX_RE.match(title)
    jr_m = _JIRA_TITLE_PREFIX_RE.match(title)
    if az_m:
        ext = ext or az_m.group(1)
        title = title[az_m.end():].strip()
        src = src or 'azure'
    elif jr_m:
        ext = ext or jr_m.group(1)
